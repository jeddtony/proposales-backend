...
let AuthService = class AuthService {
    ...
    async login(userData) {
        const findUser = await _database.DB.Users.scope('withPassword').findOne({
            where: {
                email: userData.email
            }
        });
        ...
        if (!isPasswordMatching) throw new _httpException.HttpException(409, 'Login credentials are not valid');
